...
(function () {
    ...
    function initAutoScrollProjects() {
        function setupScroller(containerId, imgId, badgeId, progressId, sections, delay) {
        ...
        setupScroller('cosmoAutoScrollBox', 'cosmoAutoScrollImg', 'cosmoSectionBadge', 'cosmoScrollProgress', [
        ...
        setupScroller('rideplusAutoScrollBox', 'rideplusAutoScrollImg', 'rideplusSectionBadge', 'rideplusScrollProgress', [
    ...
    let detailAutoScrollTimer = null;
    ...
    function startDetailAutoScroll(sections) {
        ...
        if (!box || !img)
            return;
        function stepScroll() {
            if (detailIsPaused || detailManualPaused)
                return;
        ...
        if (badge && detailSections[0])
            badge.textContent = detailSections[0];
        if (playIcon)
    ...
    function openProjectDetails(projectId, updateHash) {
        ...
        const domainBar = document.getElementById('detailDomainBar');
        const domainText = document.getElementById('detailDomainText');
        if (domainBar)
            domainBar.href = data.link || '#';
        if (domainText)
            domainText.textContent = data.domain || 'live-project.dev';
        ...
        if (githubLink)
            githubLink.href = data.github || 'https://github.com';
        ...
        if (frontendContainer)
            frontendContainer.innerHTML = renderTechChips(data.frontend);
        const backendContainer = document.getElementById('detailBackendChips');
        if (backendContainer)
            backendContainer.innerHTML = renderTechChips(data.backend);
        const featuresList = document.getElementById('detailFeaturesList');
        if (featuresList)
            featuresList.innerHTML = renderFeaturesList(data.features);
        if (mainView)
    ...
    function closeProjectDetails(updateHash) {
        stopDetailAutoScroll();
        const mainView = document.getElementById('mainPortfolioView');
        const detailsView = document.getElementById('projectDetailsView');
        if (!detailsView)
    ...
    function initProjectDetails() {
        ...
        if (cosmoBox) {
            cosmoBox.addEventListener('click', (e) => {
        ...
        if (rideplusBox) {
            rideplusBox.addEventListener('click', (e) => {
                if (e.target.closest('a'))
                    return;
        ...
        if (detailBox) {
            ...
            detailBox.addEventListener('wheel', (e) => {
                e.preventDefault();
                detailIsPaused = true;
                const img = document.getElementById('detailPreviewImg');
                if (!img)
    ...
    function initCalBookingModal() {
        ...
        triggers.forEach((btn) => {
            btn.addEventListener('click', (e) => {
                const me = e;
                if (me.ctrlKey || me.metaKey || me.shiftKey || me.which === 2 || me.button === 1)
                    return;
                e.preventDefault();
                openModal();
            });
    ...
    function initBlueprintGrid() {
        ...
        resizeCanvas();
        function drawShape(s, isDark, time) {
            ctx.save();
            ...
            switch (s.type) {
                ...
                case 'crosshair':
                    ...
                    const gap = sz * 0.95;
                    ctx.beginPath();
                    ctx.moveTo(-gap, -gap + brk);
                    ctx.lineTo(-gap, -gap);
                    ctx.lineTo(-gap + brk, -gap);
                    ctx.moveTo(gap - brk, -gap);
                    ctx.lineTo(gap, -gap);
                    ctx.lineTo(gap, -gap + brk);
                    ctx.moveTo(-gap, gap - brk);
                    ctx.lineTo(-gap, gap);
                    ctx.lineTo(-gap + brk, gap);
                    ctx.moveTo(gap - brk, gap);
            ...
            if (s.hover > 0.4) {
                ...
                ctx.fillText(`[${Math.round(s.x)},${Math.round(s.y)}]`, s.x + s.size * 1.2, s.y - s.size * 0.8);
                ctx.restore();
            }
        }
    ...
    function initCherryBlossoms() {
        ...
        function createPetalSprite(styleIdx) {
        ...
        const petalSprites = [createPetalSprite(0), createPetalSprite(1), createPetalSprite(2)].filter(Boolean);
        ...
        canvases.forEach((cvsEl) => {
            ...
            class Petal {
                constructor(initialScatter) {
                    ...
                    this.spinZ = 0;
                    this.opacity = 0;
                    this.sprite = petalSprites[0];
                    this.reset(initialScatter);
                }
                reset(initialScatter) {
